...
\begin{document}
	...
	\section{Introduzione al copyright}
		...
		\paragraph{Chi è Johann di Spira?}: \\ % Domanda incomprensibile
	...
	zanna = password
	\end{verbatim}
	Ora andiamo a modificare il file \path{conf/svnserve.conf} impostando il file precedentemente modificato come file per le password e impostando i permessi per gli utenti senza password
	\begin{verbatim}
	...
	\begin{itemize}
		\item per gli utenti senza una password è necessario accedere al \path{file conf/svnserve.conf} e modificare la riga in cui è presente ``anon-access = '' . Nel caso in cui non sia presente è necessario aggiungerla;
		\item per gli utenti con password è necessario modificare il file \path{conf/svnserve.conf} e modificare la riga in cui
		...
		si può usare il file \path{conf/authz}.
		Supponiamo di avere nel file \path{conf/passwd}:
		\begin{verbatim}
		[users]
		A = tos
		B = tos
		\end{verbatim}
		nel file \path{conf/svnserve}:
		\begin{verbatim}
		[general]
		anon-access = none
		password-db = passwd
