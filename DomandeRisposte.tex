...
\begin{document}
	...
	\section{Licenze}
		...
		\subsection{GPL v3}
			...
			La GPL v3 presenta alcune differenze con la GPL v2, la quale presenta ambiguità in alcune parti e non considera problemi che negli anni a venire sono emersi. Le principali aggiunte della GPL v3 rispetto alla v2 sono:
			\begin{itemize}
				\item \textbf{Regolamenta i brevetti software.} In Europa non è possibile richiedere brevetti per il software come su teorie matematiche. Nel corso degli anni la legge è stata modificata è permette quindi di brevettare software se questo è previsto all'interno di un prodotto completo. Ciò portò allo sfruttamento di tale norma e venne aggirato il vincolo dei brevetti software brevettando macchine con il solo scopo di brevettare il software all'interno. Nel software libero è possibile comunque rilasciare software protetto da brevetti a patto che tale codice venga rilasciato insieme ai brevetti.
				\item \textbf{Contrasta la Tivoizzazione.} Il Tivò era un apparecchio per registrare le trasmissioni televisive all'interno dell'hard disk della televisione. Tale apparecchio utilizzava Linux ma aveva un bootloader che non permetteva delle modifiche alla distribuzione utilizzata. La GPL v3 obbliga a dare all'utente tutto il materiale necessario a far funzionare il prodotto se un utente vuole modificare il software.
				\item \textbf{Tratta il tema della delle protezioni digitali (DRM).} La GPL v3 afferma che rilasciando il software sotto questa licenza si prende atto di non considerare le proprie modifiche come protezioni digitali.
				...
				\item \textbf{Regola la distribuzione dei codici via internet e peer2peer.}
				\item \textbf{Permette una serie di condizioni aggiuntive (restrizioni)} ciò rende la licenza compatibile con l'Apache v2.
			\end{itemize}
			La GPL v3 è stata contestata da molti, Linus Torvalds compreso, perché:
			\begin{itemize}
				\item \textbf{Incompatibilità con GPL v2.} Poiché entrambe le licenze richiedono che un'opera derivata sia sotto la stessa licenza è impossibile combinare codice licenziato GPL v2 con altro codice GPL v3. 
				\item \textbf{Limitazioni hardware.} Lottando la Tivoizzazione la licenza che riguarda il software regolamenta anche l'hardware uscendo dall'ambito di sua competenza.
			\end{itemize}						
